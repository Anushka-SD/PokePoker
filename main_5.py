...
def start_battle(user1_id, user2_id):
    ...
    user_id_1_entry = pygame_gui.elements.UITextEntryLine(
        relative_rect=pygame.Rect((5 * screen_width // 8, 2*  screen_height //8), (5 * screen_width // 8 + 50,screen_height // 200,)), manager=manager, object_id='#user_id_1_entry')
    user_id_2_entry = pygame_gui.elements.UITextEntryLine(
        relative_rect=pygame.Rect((3 * screen_height // 8, 4 *  screen_height //8), (5 * screen_width // 8,5 * screen_width // 8 + 50)), manager=manager, object_id='#user_id_2_entry')
    ...
    while True:
        ...
        header_text = header_font.render("BATTLE", True, "Black")
...
def display_table(title, data, position):
    ...
    table.field_names = ["Poke ID", "Name", "HP", "AP", "Category"]
    
    # Add data rows to the table
    for row in data:
        table.add_row(row)
    
    # Get the formatted table string
    ...
    # Render the formatted table string
    table_font = get_font(16)
    table_lines = table_str.split('\n')
    line_height = 20
    for i, line in enumerate(table_lines):
        line_text = table_font.render(line, True, "black")
        line_rect = line_text.get_rect(topleft=(position[0], position[1] + 30 + i * line_height))
...
def render_cards_table(cards, pos):
    ...
    # Render cards
    row_y = pos[1] + 100
    card_font = get_font(card_font_size)
    for card in cards:
        card_text = card_font.render(f"ID: {card[0]} - Name: {card[1]}", True, "black")
        card_rect = card_text.get_rect(topleft=(pos[0] + cell_padding, row_y))
        SCREEN.blit(card_text, card_rect)
        row_y += card_rect.height + cell_padding
