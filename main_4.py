...
def show_user_cards(userid):


    # Render the table headers
    header_font = get_font(OPTIONS_BUTTON_TEXTSIZE)
    header_text = header_font.render("BATTLE BEGINS!!!", True, "Black")
    header_rect = header_text.get_rect(center=(screen_width // 2, 50))
    SCREEN.blit(header_text, header_rect)

    # Display the user's cards in a scrollable view with a slider
    card_font = get_font(BACK_BUTTON_TEXTSIZE//2)
    card_y = 100
    for card in cards:
        card_text = card_font.render(f"{card[0]} - {card[1]}", True, "Black")
        card_rect = card_text.get_rect(topleft=(50, card_y))
...
def render_user_cards(cards, y_start):
    
    y_position = y_start
    for card in cards:
        card_text = get_font(BACK_BUTTON_TEXTSIZE // 2).render(f"Card ID: {card[0]} - Name: {card[1]}", True, "black")
        card_rect = card_text.get_rect(midtop=(screen_width // 2, y_position))
        SCREEN.blit(card_text, card_rect)
        y_position += 50
...
def start_game(userid_1, userid_2):
    # Call the GetUserCards procedure to retrieve user cards for both user IDs
    ...
    while True:
        ...
        # Render the text for user IDs
        user_id_1_text = get_font(OPTIONS_BUTTON_TEXTSIZE).render(f"User 1 ID: {userid_1}", True, "black")
        user_id_1_rect = user_id_1_text.get_rect(midtop=(screen_width // 2, 100))
        SCREEN.blit(user_id_1_text, user_id_1_rect)

        user_id_2_text = get_font(OPTIONS_BUTTON_TEXTSIZE).render(f"User 2 ID: {userid_2}", True, "black")
        user_id_2_rect = user_id_2_text.get_rect(midtop=(screen_width // 2, 150))
        SCREEN.blit(user_id_2_text, user_id_2_rect)

        # Render the user's cards
        render_user_cards(user1_cards, 200)  # Render user 1 cards starting at y-coordinate 200
        render_user_cards(user2_cards, 400)  # Render user 2 cards starting at y-coordinate 400

        # Render other elements of the game screen
        # ...
...
def play():
    ...
    start_back_button = Button(image=None, pos=(screen_width - 200, screen_height - 200),
    ...
    while True:
        ...
        start_back_button.changeColor(PLAY_MOUSE_POS)
        start_back_button.update(SCREEN)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if start_back_button.checkForInput(PLAY_MOUSE_POS):
...
def show_purchased_card(pokemon_name,message):
    SCREEN.fill("black")  # Fill the screen with black background

    # Fetch Pokémon info
    pokemon_info = fetch_pokemon_info(pokemon_name)
    success_message = 'Card purchased successfully.'
    
    if pokemon_info and message is success_message :
        ...
        error_text = error_font.render("Error: Pokémon info not found!\n Or insufficient Balance", True, (255, 0, 0))  # Red color
...
def buy_pokemon(username, pokemon_name):
    ...
    try:
        ...
        for result in cursor.stored_results():
            ...
            show_purchased_card(pokemon_name,message)
