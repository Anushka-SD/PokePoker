...
def get_poke_ids():
    ...
    # Create text entry boxes for Poke IDs
    poke_id_1_entry = pygame_gui.elements.UITextEntryLine(relative_rect=pygame.Rect((350, 275), (300, 50)), manager=manager,
                                                          object_id='#poke_id_1_entry')
    poke_id_2_entry = pygame_gui.elements.UITextEntryLine(relative_rect=pygame.Rect((350, 350), (300, 50)), manager=manager,
                                                          object_id='#poke_id_2_entry')
    
    # Variable to track Poke ID entry completion
    poke_id_1_entered = False
    poke_id_numeric_constraint_violation = False

    while True:
        POKEID_MOUSE_POS = pygame.mouse.get_pos()
        ...
        # Render text labels
        poke_id_1_label = get_font(BACK_BUTTON_TEXTSIZE).render("Poke ID 1:", True, "White")
        poke_id_2_label = get_font(BACK_BUTTON_TEXTSIZE).render("Poke ID 2:", True, "White")
        poke_id_1_label_rect = poke_id_1_label.get_rect(midright=(poke_id_1_entry.rect.left - 10, poke_id_1_entry.rect.centery))
        poke_id_2_label_rect = poke_id_2_label.get_rect(midright=(poke_id_2_entry.rect.left - 10, poke_id_2_entry.rect.centery))
        ...
        for event in pygame.event.get():
            ...
            # Process text input events for Poke IDs
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    if not poke_id_1_entered:
                        poke_id_1 = poke_id_1_entry.get_text()
                        if is_numeric(poke_id_1):
                            print("Poke ID 1:", poke_id_1)
                            poke_id_1_entered = True
                        else:
                            print("Poke ID 1 must contain only numeric characters.")
                            poke_id_numeric_constraint_violation = True
                    else:
                        poke_id_2 = poke_id_2_entry.get_text()
                        if is_numeric(poke_id_2):
                            print("Poke ID 2:", poke_id_2)
                            # Perform actions with Poke IDs
                            return poke_id_1, poke_id_2
                        else:
                            print("Poke ID 2 must contain only numeric characters.")
                            poke_id_numeric_constraint_violation = True
...
def start_battle(user1_id, user2_id):
    ...
    while True:
        ...
        if current_page == 'battle':
            ...
            # Call the function to handle Poke ID inputs
            poke_id_1, poke_id_2 = get_poke_ids()
            # Now you have the Poke IDs, you can perform further actions or start the battle
            print("Poke IDs for battle:", poke_id_1, poke_id_2)
            # Start the battle using the Poke IDs
            # start_battle_with_poke_ids(poke_id_1, poke_id_2)  # Uncomment this line if you have a function to start the battle with Poke IDs
